...
Camera parameters was controlled using the video4Linux ver. 2 API and driver\cite{v4l}. A resolution of 1280x720 with a 30 FPS was chosen. The resolution was kept high in order to have a rich details in the image and therefore making detecting bricks more stable with a cost of increased computation time due to large image sizes. 
...
Camera calibration was done using the camera\_calibration ROS package\cite{camera_calibration}. Using this tool the intrinsic parameters of the camera was calculated from images of a 9x7 chequerboard held in front of the camera in 73 different positions. Rectification of the image was then performed using the image\_proc ROS package\cite{image_proc}.
